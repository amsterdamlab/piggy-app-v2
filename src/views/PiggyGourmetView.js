...
function openGourmetCheckoutModal(offer, userStats) {
  ...
  const calculateTotals = () => {
    ...
    if (useWallet && userWallet > 0 && remainingAfterBonus > 0) {
      ...
      ">
        ...
        <div style="display:flex; align-items:center; justify-content:space-between; padding:18px 20px; border-bottom:1px solid #f1f5f9; background:white; flex-shrink:0;">
          <div style="display:flex; align-items:center; gap:10px;">
            <div style="width:36px; height:36px; border-radius:10px; background:#fce7ed; color:#be1260; display:flex; align-items:center; justify-content:center;">
        ...
        <div style="flex:1; overflow-y:auto; padding:20px; -webkit-overflow-scrolling:touch; display:flex; flex-direction:column; gap:18px;">
          ...
          <div style="background:#f8fafc; border:1px solid #e2e8f0; border-radius:16px; padding:16px; display:flex; flex-direction:column; gap:10px;">
            ...
            <div style="border-top:1px dashed #e2e8f0; padding-top:10px; display:flex; justify-content:space-between; align-items:center;">
              <span style="font-size:0.82rem; font-weight:700; color:#64748b;">Precio del Combo:</span>
          ...
          <div style="display:flex; flex-direction:column; gap:10px;">
            ...
            ${isBonusEligible && userBonus > 0 ? `
              <div style="background:#fffbeb; border:1.5px solid ${useBonus ? '#f59e0b' : '#fde68a'}; border-radius:14px; padding:14px; display:flex; align-items:center; justify-content:space-between;">
                <div style="display:flex; align-items:center; gap:10px;">
                  <div style="font-size:1.3rem;">🎁</div>
                  <div>
                    <div style="font-size:0.88rem; font-weight:800; color:#78350f;">Bono de Consumo</div>
                    <div style="font-size:0.75rem; color:#b45309; font-weight:600;">Descuento aplicable: -${formatGourmetPrice(appliedBonus)}</div>
                  </div>
                </div>
                <label style="position:relative; display:inline-block; width:44px; height:24px; cursor:pointer;">
                  <input type="checkbox" id="toggle-bonus" ${useBonus ? 'checked' : ''} style="opacity:0; width:0; height:0;">
                  <span style="position:absolute; inset:0; background:${useBonus ? '#f59e0b' : '#cbd5e1'}; border-radius:24px; transition:0.2s;"></span>
            ...
            ` : (!isBonusEligible && userBonus > 0 ? `
              <div style="background:#f8fafc; border:1px dashed #cbd5e1; border-radius:14px; padding:12px; font-size:0.78rem; color:#64748b;">
                ℹ️ Los bonos aplican para pedidos de $150.000 o más.
              </div>
            ` : '')}

            <!-- Option B: Wallet Balance -->
            ${userWallet > 0 ? `
              <div style="background:#f0fdf4; border:1.5px solid ${useWallet ? '#10b981' : '#bbf7d0'}; border-radius:14px; padding:14px; display:flex; align-items:center; justify-content:space-between;">
                <div style="display:flex; align-items:center; gap:10px;">
                  <div style="font-size:1.3rem;">💳</div>
                  <div>
                    <div style="font-size:0.88rem; font-weight:800; color:#065f46;">Pagar con Saldo Wallet</div>
                    <div style="font-size:0.75rem; color:#047857; font-weight:600;">Disponible: ${formatGourmetPrice(userWallet)}</div>
                  </div>
                </div>
                <label style="position:relative; display:inline-block; width:44px; height:24px; cursor:pointer;">
                  <input type="checkbox" id="toggle-wallet" ${useWallet ? 'checked' : ''} style="opacity:0; width:0; height:0;">
                  <span style="position:absolute; inset:0; background:${useWallet ? '#10b981' : '#cbd5e1'}; border-radius:24px; transition:0.2s;"></span>
                  <span style="position:absolute; top:2px; left:${useWallet ? '22px' : '2px'}; width:20px; height:20px; background:white; border-radius:50%; transition:0.2s;"></span>
                </label>
              </div>
            ` : ''}
          ...
          <div style="background:#f8fafc; border:1px solid #e2e8f0; border-radius:16px; padding:16px; display:flex; flex-direction:column; gap:8px; font-size:0.85rem;">
            ...
            ${appliedBonus > 0 ? `
              <div style="display:flex; justify-content:space-between; color:#b45309; font-weight:700;">
            ...
            ${appliedWallet > 0 ? `
              <div style="display:flex; justify-content:space-between; color:#047857; font-weight:700;">
                ...
                <span>-${formatGourmetPrice(appliedWallet)}</span>
              </div>
            ` : ''}
  ...
  const attachModalListeners = () => {
    ...
    document.getElementById('toggle-wallet')?.addEventListener('change', (e) => {
    ...
    document.getElementById('btn-confirm-gourmet-order')?.addEventListener('click', async () => {
      ...
      // If user pays part with wallet, register wallet debit request
      if (appliedWallet > 0) {
        try {
          await createWalletRequest({
            request_type: 'gourmet_payment',
            amount: appliedWallet,
            notes: `Pago parcial Combo ${offer.name}`
          });
        } catch (e) {
          console.warn('Could not register wallet debit:', e);
        }
      }

      // Generate WhatsApp link
      const waUrl = buildGourmetCheckoutWhatsAppLink({
        offer,
        appliedBonus,
        appliedWallet,
        cashDue,
